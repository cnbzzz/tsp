...
public class Gbt32960Msg2Bytes {
    ...
    public static byte[] uploadRealTime2Bytes(UploadRealTimeDto data){
        ...
        if(vehError != null){
            byte[] bytes = vehError2Bytes(vehError, data.getFullCellErrorList(), data.getDriveMotorErrorList(), data.getEngineErrorList(), data.getOtherErrorList());
            buf.writeBytes(bytes);
        }
...
//        buf.writeByte((byte) 6);
    ...
    public static byte[] vehError2Bytes(TVehError data, List<Long> fullCellErrorList, List<Long> driveMotorErrorList, List<Long> engineErrorList, List<Long> otherErrorList){
        Preconditions.checkArgument(data != null, "data is null");
        ...
        writeErrorItem(fullCellErrorList, buf);

        buf.writeByte(data.getDriveMotorErrorCount().byteValue());
        writeErrorItem(driveMotorErrorList, buf);

        buf.writeByte(data.getEngineErrorCount().byteValue());
        writeErrorItem(engineErrorList, buf);

        buf.writeByte(data.getOtherErrorCount().byteValue());
        writeErrorItem(otherErrorList, buf);

        return buf.getBytes();
    }

    private static void writeErrorItem(List<Long> list, Gbt32960WriteByteBuf buf) {
