...
//                == VehConstant.CHARGE_STATUS_NOCHARGE) {
  ...
  //      }
    ...
    public void randomError() {
        int errIdx = new Random().nextInt(4);

        realTimeDto.getVehError().setMaxErrorLevel(1);
        if(errIdx == 0){
            realTimeDto.getVehError().setOtherErrorCount(1);
            realTimeDto.setOtherErrorList(Arrays.asList(1l));
        }
        if(errIdx == 1){
            realTimeDto.getVehError().setDriveMotorErrorCount(1);
            realTimeDto.setDriveMotorErrorList(Arrays.asList(1l));
        }
        if(errIdx == 2){
            realTimeDto.getVehError().setEngineErrorCount(1);
            realTimeDto.setEngineErrorList(Arrays.asList(1l));
        }
        if(errIdx == 3){
            realTimeDto.getVehError().setFullCellErrorCount(1);
            realTimeDto.setFullCellErrorList(Arrays.asList(1l));
        }
